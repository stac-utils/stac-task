...
def test_stac_in_stac_out_task(data_path: Callable[[str], Path]) -> None:
    item = Item(
        id="an-id",
        geometry=None,
        bbox=None,
...
def test_stac_out_task(data_path: Callable[[str], Path]) -> None:
    assert (
        len(
...
def test_to_item_test(data_path: Callable[[str], Path]) -> None:
    assert (
        len(
